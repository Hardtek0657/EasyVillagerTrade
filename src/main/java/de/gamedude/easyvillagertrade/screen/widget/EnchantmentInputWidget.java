...
public class EnchantmentInputWidget extends TextFieldWidget {
    ...
    private String getPossibleEnchantmentNameOrElse(String input) {
        ...
        return enchantmentName != null
            ? enchantmentName.substring(input.length())
            : "";
    }
    ...
    public GuiNavigationPath getNavigationPath(GuiNavigation navigation) {
        if (navigation instanceof GuiNavigation.Tab) setEnchantmentText();
    ...
    private void setEnchantmentText() {
        setText(
            StringUtils.capitalize(
                getText() + ((suggestion == null) ? "" : suggestion)
            )
        );
    }
}
